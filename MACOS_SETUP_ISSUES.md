...
**Fix:** Edit `openwebui_cg/backend/open_webui/env.py` and add this line around line 114 (near `WEBUI_FAVICON_URL`):
...
**Critical:** Create the local `.env` file. Vite reads from `langbuilder_cg/.env`, not the root `.env`:
...
```bash
rm langbuilder_cg/src/backend/base/langbuilder/langbuilder.db
```
...
| Add WEBUI_NAME to env.py | Code bug - variable imported but not defined | Add line to `openwebui_cg/backend/open_webui/env.py` |
| Install y-protocols | Missing npm dependency for TipTap editor | `npm install y-protocols --legacy-peer-deps` |
| Create langbuilder_cg/.env | Vite reads wrong .env file, proxy fails | Create file with `VITE_PROXY_TARGET` |
...
```bash
cd openwebui_cg && npm install y-protocols --legacy-peer-deps
```

### Vite proxy returns 500 errors / ECONNREFUSED
The `langbuilder_cg/.env` file is missing or doesn't have `VITE_PROXY_TARGET`:
```bash
echo "VITE_PROXY_TARGET=http://localhost:8002" > langbuilder_cg/.env
```

### "table publish_record already exists"
```bash
cd langbuilder_cg && uv run alembic stamp head
```
